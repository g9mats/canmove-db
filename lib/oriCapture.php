...
class oriCapture {
	...
	function insert($db, $aid, $time, $lat, $long, $loc, $oid, $rem) {
		$res = $db->query("select nextval('d_ori_capture_capture_id_seq') id");
		$this->capture_id=$res[0]['id'];
		$db->execute(
			"insert into d_ori_capture (capture_id, animal_id,
	...
	function delete($db, $cid) {
		$db->execute(
			"delete from d_ori_capture where capture_id = $1",
			array($cid));
	}
