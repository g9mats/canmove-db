...
class oriAnimal {
	...
	function insert($db, $did, $tid, $tsn, $tax, $rem) {
		$res = $db->query("select nextval('d_ori_animal_animal_id_seq') id");
		$this->animal_id=$res[0]['id'];
		$db->execute(
			"insert into d_ori_animal (animal_id, dataset_id, animal,
				itis_tsn, taxon, remark)
				values ($1, $2, $3, $4, $5, $6)",
	...
	function delete($db, $aid) {
		$db->execute(
			"delete from d_ori_animal where animal_id = $1",
			array($aid));
	}
