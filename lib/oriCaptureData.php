...
class oriCaptureData {
	...
	function insert($db, $cid, $ordno, $datid, $value) {
		$res = $db->query("select nextval('d_ori_capture_data_capture_data_id_seq') id");
		$this->capture_data_id=$res[0]['id'];
		$db->execute(
			"insert into d_ori_capture_data (capture_data_id, capture_id,
