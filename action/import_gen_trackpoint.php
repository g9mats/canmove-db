...
This script loads GEN tracking data from tab separated text file into staging
area.
...
The actual Trackpoint data is loaded into table l_gen_trackpoint for
subsequent validation.
*/

function import_gen_trackpoint ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata=
	"insert into l_gen_trackpoint (dataset_id,device_id,period,version";
$sql_insdata2=$dataset_id.",".$device_id.",'".$period."',".$version;
...
// Delete any previously loaded data from l_gen_trackpoint
$res=$db->execute($sql_deldata,array($dataset_id,$device_id,$period,$version));
...
foreach ($headers as $htext) {
	...
	// Define destination column for data in table l_gen_trackpoint
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_gen_trackpoint
