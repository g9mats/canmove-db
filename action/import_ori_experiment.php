...
This script loads ORI experiment data from tab separated text file into staging
area.
...
The actual Experiment data is loaded into table l_ori_experiment for
subsequent validation.
*/

function import_ori_experiment ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata=
	"insert into l_ori_experiment (dataset_id";
...
// Delete any previously loaded data from l_ori_experiment
...
foreach ($headers as $htext) {
	...
	// Define destination column for data in table l_ori_experiment
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_ori_experiment
