...
This script loads ORI count data from tab separated text file into staging
area.
...
The actual Count data is loaded into table l_ori_count for
subsequent validation.
*/

function import_ori_count ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata=
	"insert into l_ori_count (dataset_id,version";
$sql_insdata2=$dataset_id.",".$version;
if (!in_array("Experiment No",$headers)) {
	$sql_insdata.=",experiment_no";
	$sql_insdata2.=",1";
}
if (!in_array("Phase No",$headers)) {
	$sql_insdata.=",phase_no";
	$sql_insdata2.=",1";
}
...
// Delete any previously loaded data from l_ori_count
$res=$db->execute($sql_deldata,array($dataset_id,$version));
...
foreach ($headers as $htext) {
	// Check if header is one of the count columns and treat specially
	if (strncmp($htext,"S",1)==0 && is_numeric(substr($htext,1)))
		$res=$db->query($sql_selvar,array("S[n]"));
	else
	...
	// Define destination column for data in table l_ori_count
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_ori_count
