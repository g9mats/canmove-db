...
This script loads GEN capture data from tab separated text file into staging
area.
The first row should contain agreed headers defined in table r_data
but do not need to appear in a certain order. The column information is loaded
into l_column. If an unknown header is found the script will terminate.
The column definitions will also be loaded into p_column if this is the
first load of this dataset. This will set the column order for subseguent
data loading.

The actual Animal, Track and Capture data is loaded into table l_gen_capture
for subsequent validation.
*/

function import_gen_capture ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata="insert into l_gen_capture (dataset_id";
$sql_insdata2=$dataset_id;

// Delete any old column definitions
$res=$db->execute($sql_delvar,array($dataset_id));
//echo pg_affected_rows($res)." column definitions deleted<br/>";

// Delete any previously loaded data from l_gen_capture
$res=$db->execute($sql_deldata,array($dataset_id));
//echo pg_affected_rows($res)." data rows deleted<br/>";
...
foreach ($headers as $htext) {
	...
	// Define destination column for data in table l_gen_capture
	if ($row['column_type']=="var") {
		$varnum++;
		$lname=$row['load_name'].$varnum;	// c1,...
	} else
		$lname=$row['load_name'];
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_gen_capture
		$res=$db->execute($sql_insdata,$data);
		$datanum++;
	}
}
fclose ($lfile);
