...
This script loads ORI estimation data from tab separated text file into staging
area.
...
The actual Estimation data is loaded into table l_ori_estimation for
subsequent validation.
*/

function import_ori_estimation ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata=
	"insert into l_ori_estimation (dataset_id,version";
...
// Delete any previously loaded data from l_ori_estimation
...
foreach ($headers as $htext) {
	...
	// Define destination column for data in table l_ori_estimation
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_ori_estimation
