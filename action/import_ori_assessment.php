...
This script loads ORI assessment data from tab separated text file into staging
area.
...
The actual Assessment data is loaded into table l_ori_assessment for
subsequent validation.
*/

function import_ori_assessment ($fname, $separator, $file_id) {
if ($separator=="tab")
	$schar="	";
else
	$schar=",";
...
$sql_insdata=
	"insert into l_ori_assessment (dataset_id";
$sql_insdata2=$dataset_id;
if (!in_array("Assessment No",$headers)) {
	$sql_insdata.=",assessment_no";
	$sql_insdata2.=",1";
}
...
// Delete any previously loaded data from l_ori_assessment
...
foreach ($headers as $htext) {
	...
	// Define destination column for data in table l_ori_assessment
...
while (($buffer = fgets ($lfile)) !== FALSE) { // Read rest of file
	...
	if ($notnull > 0) {
		// Insert one data row into table l_ori_assessment
